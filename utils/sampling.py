...
from scipy.special import gammaln, digamma, polygamma,logsumexp, kve, kv
import numpy as np
import numpy.matlib
...
def simulation_mixed_data(cont_law,disc_law,cont_params,discr_params,n_samples):
	...
	obs = []
	disc_obs = []
	n_clusters = [int(np.round(n_samples * cont_params['proportions'][c])) for c in range(n_components)]
	...
	for c in range(n_components):
		...
		if cont_law=='Gaussian':
			x = np.random.multivariate_normal(cont_params['locations'][c], cont_params['scales'][c], (n_samples,))
			x = x[0:n_clusters[c]]

		elif cont_law=='Student':
			y = np.random.chisquare(cont_params['dofs'][c], n_samples) / cont_params['dofs'][c]
			z = np.random.multivariate_normal(np.zeros(n_features_cont), cont_params['scales'][c], (n_samples,))
			x = cont_params['locations'][c] + z / np.sqrt(y)[:, None]
			x = x[0:n_clusters[c]]

		elif cont_law=='SAL':
			Y = np.random.multivariate_normal(np.zeros(n_features_cont), cont_params['scales'][c], (n_samples,))
			W = np.random.exponential(scale=1.,size=(n_samples,1)) 
		...
		for j,(type_var) in enumerate(disc_law):
			if type_var != 'None':
				if type_var=='Bernoulli':
					var_disc = np.random.binomial(1,discr_params[type_var+str(j)][c],(n_clusters[c],1))
					discr_array.append(var_disc)
				elif type_var=='Multinomial':
					cat = np.random.multinomial(1,discr_params[type_var+str(j)][c],size=(n_clusters[c],1))
					var_disc = np.argwhere(cat==1)[:,2]+1
					discr_array.append(var_disc.reshape(n_clusters[c],1))
				elif type_var=='Poisson':
					var_disc = np.random.poisson(lam=discr_params[type_var+str(j)][c],size=(n_clusters[c],1))
					discr_array.append(var_disc)
				else:
